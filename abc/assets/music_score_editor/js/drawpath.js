...
function updatePath(e) {
  ...
  if (targetType == "slur") {
    ...
    $(selectRect).attr("x", e.offsetX / scale - dragRectWidth / 2);
    $(selectRect).attr("y", e.offsetY / scale - dragRectWidth / 2 - 59);
  } else {
  ...
  if (targetType == "slur") {
    //普通连音线的拖动
    //		M61.29 82.00c9.2 4.4 53.3 4.4 62.5 0.0
    //		v0.3c-11.1 6.2 -51.5 6.2 -62.5 -0.3

    //		M66.29 66.64c8.3 -13.9 47.0 -32.9 55.3 -27.2
    //		v-0.3       c-10.0 -7.4 -45.3 11.6 -55.3 27.5

    //		M65.29 94.60c8.7 7.2 49.8 -1.6 58.5 -12.6
    //		v0.3c-10.5 12.7 -48.1 21.5 -58.5 12.3

    //		M65.29 94.60c8.8 11.7 50.1 2.9 58.9 -12.6
    //		v0.3c-10.5 17.2 -48.4 26.1 -58.9 12.3

    var ct1X = control1.x - startPoint.x;
    var ct1Y = control1.y - startPoint.y;
    var ct2X = control2.x - startPoint.x;
    var ct2Y = control2.y - startPoint.y;
    var eX = endPoint.x - startPoint.x;
    var eY = endPoint.y - startPoint.y;
...
function drawrectPath(svg, x, y, type, target) {
  ...
  rect.setAttribute("stroke", "#0E518F");
...
function changeCuntomSlurSetting(ssIstart, esIstart) {
  ...
  var newStartStr =
    ...
    seq +
    "[sx:" +
    (old.startPoint.x + startxDis / scale).toFixed(1) +
    ",sy:" +
    (old.startPoint.y + startyDis / scale).toFixed(1) +
    ",c1x:" +
    (old.control1.x + control1xDis / scale).toFixed(1) +
    ",c1y:" +
    (old.control1.y + control1yDis / scale).toFixed(1) +
    ",c2x:" +
    (old.control2.x + control2xDis / scale).toFixed(1) +
    ",c2y:" +
    (old.control2.y + control2yDis / scale).toFixed(1) +
    ']"';
  var newEndStr =
    '")slur-' +
    seq +
    "[x:" +
    (old.endPoint.x + endxDis / scale).toFixed(1) +
    ",y:" +
    (old.endPoint.y + endyDis / scale).toFixed(1) +
    ']"';
  ...
  if (selectedStartNote != null && selectedStartNote.istart != ssIstart) {
    ...
    var sufStr = content
      .substring(selectedStartNote.istart)
      .replace(startReg, "");
  ...
  if (selectedEndNote != null && selectedEndNote.istart != esIstart) {
    ...
    newContent = newContent.replace(
      startReg,
      '"(slur-' + seq + '[sx:0,sy:0,c1x:0,c1y:0,c2x:0,c2y:0]"'
    );
    $("#source").val(newContent);
    doLog();
    src_change();
...
function drawBracketGchPath(
  ...
  var vLineDStr =
    "M" + startPoint.x + " " + M_y + "v5.00m" + distance + " 0.00v-5.00";
  if (drawType == "start") {
    //只画开始的竖线，结束的在下一行
  ...
  var hLineDStr =
    "M" + startPoint.x + " " + (startPoint.y - 10) + "l" + distance + " 0.00";
  hLine.setAttribute("d", hLineDStr);
...
function delBracketGch(seq, type) {
  var selObj = $(
    "text[type='bracketgch'][seq='" + seq + "'][subtype='" + type + "']"
  );
  if ($(selObj).length > 0) {
...
const svgPath = (points, command) => {
  ...
  var s = `<path type="XL" d="${d}" fill="none" stroke-width="1.5" stroke="#0E518F" />`;
  return s;
};
