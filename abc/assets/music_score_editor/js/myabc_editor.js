...
const speedTxtList = [
  ...
  { type: "staff", val: "60", txt: "Larghtto", title: '小广板', min: 60, max: 65 },
  { type: "staff", val: "66", txt: "Andante", title: '行板', min: 66, max: 68 },
  { type: "staff", val: "69", txt: "Andantino", title: '小行板', min: 69, max: 87 },
  { type: "staff", val: "88", txt: "Moderato", title: '中板', min: 88, max: 100 },
  ...
  { type: "easy", val: "60", txt: "小广板", min: 60, max: 65 },
  { type: "easy", val: "66", txt: "行板", min: 66, max: 68 },
  { type: "easy", val: "69", txt: "小行板", min: 69, max: 87 },
  { type: "easy", val: "88", txt: "中板", min: 88, max: 100 },
...
var staffTypes = {
  ...
  string:
    ...
    "%%vsetting_start\n" +
    "%%score [1 |2 |3 |4]\n" +
    'V:1 treble nm="小提琴1" snm="小提1"\n' +
    "%%MIDI program 40\n" +
    'V:2 treble nm="小提琴2" snm="小提2"\n' +
    "%%MIDI program 40\n" +
    'V:3 alto  nm="中提琴" snm="中提"\n' +
    "%%MIDI program 41\n" +
    'V:4 bass nm="大提琴" snm="大提"\n' +
    "%%MIDI program 42\n" +
    "%%vsetting_end\n" +
    "V:1\n" +
    "z,8|z,8|z,8|z,8|z,8|z,8|z,8|z,8|$ z,8 | z,8 | z,8 | z,8 | z,8 | z,8 | z,8 | z,8 |\n" +
  ...
  wind:
    ...
    "%%vsetting_start\n" +
    "%%score [1 |2 |3 |4]\n" +
    'V:1 treble nm="长笛" snm="长笛"\n' +
    "%%MIDI program 73\n" +
    'V:2 treble nm="双簧管" snm="双簧管"\n' +
    "%%MIDI program 68\n" +
    'V:3 alto  nm="单簧管" snm="单簧管"\n' +
    "%%MIDI program 71\n" +
    'V:4 bass nm="大管" snm="大管"\n' +
    "%%MIDI program 70\n" +
    "%%vsetting_end\n" +
    "V:1\n" +
    "z,8|z,8|z,8|z,8|z,8|z,8|z,8|z,8|$ z,8 | z,8 | z,8 | z,8 | z,8 | z,8 | z,8 | z,8 |\n" +
...
$(document).ready(function () {
  ...
  $(".tone-ruler").width(
    $(".pianoKeyboard .keyRow.white .key").outerWidth() * 7
  );
  //谱例类型
  ...
  $(".showmeasurebox").click(function () {
    showMeasurebox("source");
  });
  //是否有弱起小节
  $("#isR").click(function () {
    if ($(this).is(":checked")) {
  ...
  // 音程尺拖动事件
  $(".tone-ruler, .dshx-ruler, .xshx-ruler")
  ...
  if (!broserOk) {
    ...
    document.write(
      "您的浏览器不支持本网站,请下载<a href='https://www.google.cn/chrome/'>谷歌（Chrome）浏览器</a>或<a href='http://www.firefox.com.cn/download/'>火狐（Firefox）浏览器</a>"
    );
    return;
  }
  ...
  $("#pianoKeysWhite .key").each(function (idx, val) {
    ...
    name =
      ...
      (nameArr.length > 1
        ? "<i class='" +
          ("ABCDEFG".indexOf(nameArr[0]) != -1 ? "down" : "") +
          "'>" +
          nameArr[1] +
          "</i>"
        : "");
  ...
  $("#M_den").on("change", function () {
    setMValue();
    src_change(changeMValue);
    //		changeMValue()
  });
  // 单位
  ...
  $("#Q_V").on("input", function () {
    ...
    if (qv < 46) {
      ...
      $("#plsxspeedtype_en").val("40");
    } else if (qv < 52) {
      $("#plsxspeedtype").val("46");
      $("#plsxspeedtype_en").val("46");
    } else if (qv < 56) {
      $("#plsxspeedtype").val("52");
      $("#plsxspeedtype_en").val("52");
    } else if (qv < 60) {
      $("#plsxspeedtype").val("56");
      $("#plsxspeedtype_en").val("56");
    } else if (qv < 66) {
      $("#plsxspeedtype").val("60");
      $("#plsxspeedtype_en").val("60");
    } else if (qv < 69) {
      $("#plsxspeedtype").val("66");
      $("#plsxspeedtype_en").val("66");
    } else if (qv < 88) {
      $("#plsxspeedtype").val("69");
      $("#plsxspeedtype_en").val("69");
    } else if (qv < 108) {
      $("#plsxspeedtype").val("88");
      $("#plsxspeedtype_en").val("88");
    } else if (qv < 132) {
      $("#plsxspeedtype").val("108");
      $("#plsxspeedtype_en").val("108");
    } else if (qv < 160) {
      $("#plsxspeedtype").val("132");
      $("#plsxspeedtype_en").val("132");
    } else if (qv < 184) {
      $("#plsxspeedtype").val("160");
      $("#plsxspeedtype_en").val("160");
    } else if (qv < 208) {
      $("#plsxspeedtype").val("184");
      $("#plsxspeedtype_en").val("184");
    } else if (qv >= 208) {
      $("#plsxspeedtype").val("208");
      $("#plsxspeedtype_en").val("208");
    }
    set("Q:", qdesc + q + "=" + qv);
    $("#NOTE_Q_V").val(qv);
  ...
  $(".pitchbtn").click(function () {
    var selectedNotes = $(
      ".selected_text[type*='hd'],.selected_text[type='note']"
    );
  ...
  $("#scoreEditorPianoKeyboard .key")
    .mousedown(function () {
      ...
      if (parentGroup.substr(0, 1) == "C") {
        ...
        $("div[group='C" + parentGroup.substr(1, 1) + "']").addClass(
          "group-active"
        );
      }
      // 播放
  ...
  $(document)
    .on("keydown", function (event) {
      ...
      //歌词编辑框
      if (
        obj[0].tagName.toUpperCase() == "DIV" &&
        ($(obj).hasClass("editor-div") || $(obj).hasClass("editor-repeatbar"))
      ) {
        if (event.which === 9) {
          //tab键
      ...
      //    如果当前焦点是文本框或文本域则正常走
      if (
        obj[0].tagName.toUpperCase() == "TEXTAREA" ||
        obj[0].tagName.toUpperCase() == "INPUT"
      ) {
        return;
      }
      ...
      if (event.which === 37) {
        ...
        if (selectTextObj.length > 0) {
          var istart = parseInt(
            $(".selected_text,.select_text_g").attr("istart")
          );
          ...
          for (var i = istart - 1; i > 0; i--) {
            ...
            if (s && s.v == currS.v) {
              if (s.type == 8 || s.type == 10) {
                $(selectTextObj)
                  .removeClass("selected_text")
                  .removeClass("select_text_g")
                  .removeAttr("style");
  ...
  $("#changenodespeed").click(function () {
    $("#QC_div .modal-content").css(
      "left",
      ($(window).width() - $("#QC_div .modal-content").width()) / 2
    );
    $("#speedtype").val("node");
    $("#QC_div").modal();
  });
  ...
  $("#xnrs").click(function () {
    ...
    $("#lty_txt").val(val);
    $("#LTY_div .modal-content").css(
      "left",
      ($(window).width() - $("#LTY_div .modal-content").width()) / 2
    );
    $("#LTY_div").modal();
  });
  // 禁用输入设备
  ...
  $("#nometer").click(function () {
    ...
    if ($("#nometer")[0].checked) {
        ...
        for (var i = 0; i < syms.length; i++) {
          if (syms[i] && syms[i] != null) {
            if (syms[i].type == 8 || syms[i].type == 10) {
              content =
                content.substring(0, syms[i].istart) +
                '"サ"' +
                content.substr(syms[i].istart);
              break;
            }
          }
        }
      ...
      if (content.indexOf("!invisible!M:") > -1) {
        $("#source").val(
          content.replace("!invisible!M:", "M:").replace('"サ"', "")
        );
      }
    }
    abc_change();
  ...
  if ($("#svg_pic").length > 0) {
    //这里处理有时候进入页面没有调用resize事件，导致没有渲染，重新渲染一次
  }
  ...
  var hideBbs = getUrlParameter("hidebbs");
  if (hideBbs == "1") {
    $("li[data-code='bbs']").hide();
  }
  ...
  $("li[data-code='shortcutKey']").click(function () {
    console.log("快捷键说明");
    $("#SHORTCUT_div .modal-content").css(
      "left",
      ($(window).width() - $("#SHORTCUT_div .modal-content").width()) / 2
    );
    $("#SHORTCUT_div").modal();
  });
  //论坛
  ...
  $("#singleLyricColorInput").spectrum({
    showAlpha: false,
    showButtons: false,
    move: function (color) {
      setSingleLyricColor();
...
function saveHandle() {
  ...
  if ($("#pictype").val() == "mobile") {
    ...
    // options.watermark = "assets/music_score_editor/images/1111.png";
    setTimeout(function () {
            ...
            setTimeout(function () {
              ...
              svgAsPngUri(mixstaffcanvas, options, function (uri2) {
                ...
                setTimeout(function () {
                  ...
                  barLineCoorData(barLineCoor, musicType);
                  $("#jp_barlinecoor").val(
                    formatJson(JSON.stringify(barLineCoor))
                  );
...
function myplay() {
  ...
  if (str == "播放") {
    ...
    var selectNote = $(
      "text[type='hd'].selected_text,text[type='Hd'].selected_text,text[type='HD'].selected_text,text[type='note'].selected_text"
    );
    ...
    $("#playimg").attr("src", "assets/music_score_editor/images/preview_pause.png");
    changeChoice();
    // 这里做延迟主要是因为播放节奏加载延迟
    // 节拍器不用这种播放方式了
    // setTimeout(function(){
    ...
    $("#playspan").text("正在暂停");
    $("#playimg").attr("src", "assets/music_score_editor/images/preview_play.png");
    play_tune(0);
    changeChoice();
  } else if (str == "继续") {
    $("#player").addClass("active");
...
function enter(flag) {
  ...
  if (flag == "before") {
    ...
    content =
      content.substr(0, startPos + st.length) +
      "\n" +
...
function editLyric(sourceid) {
  ...
  $("#LYRIC_div .modal-content")
    .width($(window).width() - 200)
    .height($(window).height() - 100);
  $("#LYRIC_div .modal-content").css(
    "left",
    ($(window).width() - $("#LYRIC_div .modal-content").width()) / 2
  );
...
function getLyricDiv(sourceid) {
  ...
  var inputDemo =
    '<input type="text" style="width:40px;margin:1px;" node_index="nodeIndex" note_in_node_index="noteInNodeIndex" line="var_line" index="var_index" note="var_note" value="var_value" inner_line="0">';
  var lyricLineDiv = "";
  var inputBuffer = "";
  var input_count = 0;
  for (var i = 0; i < noteLyrics.length; i++) {
    ...
    for (var j = 0; j < notes.length; j++) {
      ...
      lineDemo += inputDemo
        .replace("var_index", notes[j].index)
        .replace("nodeIndex", notes[j].nodeIndex)
        .replace("noteInNodeIndex", notes[j].noteInNodeIndex)
        .replace("var_note", notes[j].note)
        .replace("var_value", "value_" + notes[j].index)
        .replace("var_line", i);
        ...
        if (hasNodeLine) {
          ...
          $(
            "input[line=" +
              ...
              word.nodeIndex +
              "][note_in_node_index=" +
              word.noteInNodeIndex +
              "]"
          ).val(word.word.replace("*", ""));
        } else {
          // 歌词没有小节线
          $("input[line=" + i + "][inner_line=" + j + "][index=" + k + "]").val(
            word.word.replace("*", "")
          );
        }
      }
...
function replaceSelected(selectText, newText) {
  ...
  var newContent =
    ...
    newText +
    content.substr(endPos, content.length);
  $("#source").val(newContent);
  abc_change();
  setSelectRange(
  ...
  if (x_matchs != null) {
    ...
    if (flag == "K:") {
      if (c_matchs != null) {
        ...
        $("#source").val(
          str.replace(c_matchs[0], c_matchs[0] + flag + " " + value + "\n")
        );
        return;
      }
    }
    ...
    if (flag == "V:") {
      if (k_matchs != null) {
        $("#source").val(
          str.replace(k_matchs[0], k_matchs[0] + flag + " " + value + "\n")
        );
        return;
      }
    }
    ...
    if (flag == "C:") {
      if (t_matchs != null) {
        $("#source").val(
          str.replace(t_matchs[0], t_matchs[0] + flag + " " + value + "\n")
        );
        return;
      }
    }
    $("#source").val(
      str.replace(x_matchs[0], x_matchs[0] + flag + " " + value + "\n")
    );
  } else {
    if (flag == "X:") {
...
function getNotesLength(nodestr) {
  ...
  var pattern =
    /\([2-9].[^\s]*\s|(\[[\^\_\=A-Za-z\/1-9,']*\])|(\^){0,1}(\_){0,1}(\=){0,1}[^\[\s\]]{1}[\,\'\/|1-9]*/g;
  var L = get("L:").split("/")[1];
  var M = get("M:").split("/")[1];
  // 计算一拍需要几个单位音符
  var bs = L / M;
      ...
      if (data != null && data != "") {
        ...
        for (var i = 0; i < files.length; i++) {
          if (
            ...
            (files[i].FILECATEGORY == "" ||
              files[i].FILECATEGORY == "stand_png")
          ) {
          ...
          if (
            (files[i].FILETYPE == "jpg" || files[i].FILETYPE == "png") &&
            (files[i].FILECATEGORY == "" ||
              files[i].FILECATEGORY == "stand_png")
          ) {
          ...
          if (
            files[i].FILETYPE == "mp3" &&
            (files[i].FILECATEGORY == "" ||
              files[i].FILECATEGORY == "stand_mp3")
          ) {
          ...
          if (
            files[i].FILETYPE == "abc" &&
            (files[i].FILECATEGORY == "" ||
              files[i].FILECATEGORY == "stand_abc")
          ) {
            existAbc = true;
...
function getFileServer() {
  file_server_url = window.AE.file_server_url || '';
  // var url = "assets/music_score_editor/fileserver.json";
  // $.ajax({
  //   type: "GET",
  //   url: url,
  //   data: "",
...
function replaceSuffix(selectText) {
  var st = selectText.replaceAll(
    /((![0-9]*!)|(![a-zA-Z]*!)|(!\>!)|(!\<\(!)|(!\<\)!)|(!\>\(!)|(!\>\)!)|\.|(\(\d)|v|u|\>|P)/g,
    ""
  );
  return st;
}
...
function resetNodeLineData() {
  ...
  for (var i = 1; i < times.length; i++) {
    ...
    var x = parseFloat(
      (hOff + xCoors[lineNum][barIndex - 1] * scale).toFixed(2)
    );
    var y = parseFloat(
      ((lineOffsets[lineNum] + yMins[lineNum]) * scale).toFixed(2)
    );
    var w = parseFloat(
      (
        (xCoors[lineNum][barIndex] - xCoors[lineNum][barIndex - 1]) *
        scale
      ).toFixed(2)
    );
...
function startPreview() {
  ...
  var tInterval = setInterval(function () {
    ...
    if (currPrevIndex != lastPrevIndex) {
      ...
      var w =
        (xCoors[lineNum][barIndex] - xCoors[lineNum][barIndex - 1]) * scale;
...
function showIECFrame() {
  $("#IEC_div .modal-content").css(
    "left",
    ($(window).width() - $("#IEC_div .modal-content").width()) / 2
  );
  setTimeout(function () {
...
function renderPaper() {
  ...
  $("#N_div .modal-content").css(
    "left",
    ($(window).width() - $("#N_div .modal-content").width()) / 2
  );
  var canvas = mergeSvg2Png();
...
function openRhythm() {
  $("#RHYTHM_div .modal-content")
    .width($(window).width() - 200)
    .height($(window).height() - 100);
  $("#RHYTHM_div .modal-content").css(
    "left",
    ($(window).width() - $("#RHYTHM_div .modal-content").width()) / 2
  );
...
function writeNumStaff(transport) {
  ...
  $("#pianoKeysWhite .key").each(function (idx, val) {
    ...
    if (noteCount == 1) {
      ...
      if (type == "b") {
        ...
        str +=
          "<div class='key-note-white-top key-note-val' val=\"" +
          numStaff.split(":")[0] +
          '">' +
          num.split(":")[0] +
          "<i class='" +
          ...
          "'/></div>";
        str +=
          "<div class='key-note-white-bottom key-note-val' val=\"" +
          numStaff.split(":")[1] +
          '">' +
          num.split(":")[1] +
          "<i class='" +
  ...
  $("#pianoKeysBlack .key").each(function (idx, val) {
    ...
    if (noteCount == 1) {
      ...
      str +=
        "<div class='key-note-black-mid key-note-val' val=\"" +
        numStaff +
        '">' +
        num;
      if (type == "b") {
      ...
      if (type == "b") {
        ...
        str +=
          "<div class='key-note-black-sub key-note-black-top key-note-val' val=\"" +
          numStaff.split(":")[0] +
          '">' +
          num.split(":")[0] +
          "<i class='" +
          ...
          "' /></div>";
        str +=
          "<div class='key-note-black-sub key-note-black-bottom key-note-val' val=\"" +
          numStaff.split(":")[1] +
          '">' +
          num.split(":")[1] +
          "<i class='" +
...
function selectspeedoption(obj) {
  $(obj.previousSibling).prop(
    "checked",
    !$(obj.previousSibling).prop("checked")
  );
}
function openSpeedFile() {
  $("#SAVE_div .modal-content").css(
    "left",
    ($(window).width() - $("#SAVE_div .modal-content").width()) / 2
  );
...
function numstaffpress() {
  ...
  if (e.keyCode == 13) {
    ...
    val = val.replace(num_pattern, function (m, p1, p2, p3) {
      return notes[parseInt(m) - 1];
    });
...
function changestaffspeed() {
  $("#QC_div .modal-content")
    .css("left", ($(window).width() - $("#QC_div .modal-content").width()) / 2)
    .css("top", 100);
  $("#speedtype").val("staff");
  setSpeedSelected();
}
...
function openZDiv() {
  $("#Z_div .modal-content")
    .css("left", ($(window).width() - $("#Z_div .modal-content").width()) / 2)
    .css("top", 100);
}
// 删除速度
...
function saveSpeedFile(speed) {
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.indexOf("成功") > -1) {
        $("input[name='demospeedinput'][value='" + speed + "']").attr(
          "saved",
          "1"
        );
        $(".loading,.loading-box").remove();
...
function saveOtherSpeedFile() {
  ...
  var speedinputs = $(
    "input[name='demospeedinput'][value!='speedval'][saved='0']"
  );
...
function openPreviewPic() {
  $("#PREVIEWPIC_div .modal-content")
    .width($(window).width() - 200)
    .height($(window).height() - 120);
  $("#PREVIEWPIC_div .modal-content").css(
    "left",
    ($(window).width() - $("#PREVIEWPIC_div .modal-content").width()) / 2
  );
  $("#picpreview_paper_div").height(
    $("#PREVIEWPIC_div .modal-content").height() - 120
  );
  $("#picpreview_pic_div").height(
    $("#PREVIEWPIC_div .modal-content").height() - 120
  );
  $("#abc").val($("#source").val());
  //
...
function setPageWidth(sourceid) {
  ...
  if (pageWidthStr != "") {
    if (lastScale < scale) {
      content = content.replace(
        pageWidthStr,
        pageWidthStr.replace(pageWidth, parseInt(pageWidth) * scale)
      );
...
function showMyStaff() {
  $("#MY_STAFF_div .modal-content").css(
    "left",
    ($(window).width() - $("#MY_STAFF_div .modal-content").width()) / 2
  );
  ...
  $("#staffListIframe")[0].contentWindow.listMyAbcTypes(
    personId,
    file_server_url
  );
...
function setDoPosition() {
  ...
  if (matchs == null || matchs.length == 0) {
    var lowerOrHigher = getLowerOrHigher();
    $("input[name='doPos'][value='" + lowerOrHigher + "']").prop(
      "checked",
      "checked"
    );
...
function setMValue() {
  ...
  var idx = get2N($("#M_den").val());
  var oldSpVal = $("#selectSpeedImg").attr("speed").split("/")[1];
  var newSpVal = $("#M_den").val();
  var n = newSpVal / oldSpVal;
  $("#Q_V").val($("#Q_V").val() * n);
  selectSpeed(parseInt(idx) + 1, "1/" + $("#M_den").val());
  $("#M").change();
}
...
function setNodeMValue() {
  $("#node_M").val(
    "[M:" + $("#node_M_mol").val() + "/" + $("#node_M_den").val() + "]"
  );
}
// 打开分声部导出页
function openVoicepartExport() {
  $("#VOICEPART_div .modal-content").css(
    "left",
    ($(window).width() - $("#VOICEPART_div .modal-content").width()) / 2
  );
  initVoicePart(content_vue, $("#source").val());
}
// 打开音量设置页
function openVolSetting() {
  $("#VOL_SETTING_div .modal-content").css(
    "left",
    ($(window).width() - $("#VOL_SETTING_div .modal-content").width()) / 2
  );
  initVoicePart(content_vue, $("#source").val());
...
function initVoicePart(that, abcContent) {
  that.voicePart = new VoicePart(
    abcContent ? abcContent : that.abcContent,
    function (partList) {
...
function exportAllVoice() {
  ...
  addInput2Form(
    "exportAllVoiceForm",
    "staffCount",
    content_vue.voicePart.partList.length
  );
...
function exportAllVoice_(partList, partAbc) {
  ...
  addInput2Form(
    "exportAllVoiceForm",
    "partName" + exoprtStaffIndex,
    partAbc.name
  );
  addInput2Form(
    "exportAllVoiceForm",
    "partAbc" + exoprtStaffIndex,
    partAbc.abc
  );
  src_change(function () {
    //把五线谱数据放到表单里
...
function exportVoice() {
  ...
  if (
    ...
    selectedIndex[0] == content_vue.voicePart.partList.length - 1
  ) {
    partAbc = $("#source").val();
  }
...
function exportAbc2Pdf(sourceid, toImage) {
  ...
  if (content.indexOf("%%leftmargin") > -1) {
    content = content.replace(
      /%%leftmargin.*/g,
      "%%leftmargin " + (leftmargin * dpi) / 2.54 + ""
    );
  } else {
  ...
  if (content.indexOf("%%rightmargin") > -1) {
    content = content.replace(
      /%%rightmargin.*/g,
      "%%rightmargin " + (rightmargin * dpi) / 2.54 + ""
    );
  } else {
  ...
  if (content.indexOf("pageheight") < 0) {
    ...
    var options = new Object();
    //水印地址
    //options.watermark = "assets/music_score_editor/images/2222.jpg";
    ...
    setTimeout(function () {
      ...
      $.each($("#target>div>svg"), function (i, item) {
        ...
        if (firstStyle == null || firstStyle.length == 0) {
          ...
          var h1 =
            '<defs><path id="hl" class="stroke" stroke-width="1" d="m-6 0h12"></path></defs>';
          var rdots =
            '<g id="rdots" class="fill"><circle cx="0" cy="-9" r="1.2"></circle><circle cx="0" cy="-15" r="1.2"></circle></g>';
          $(item).prepend($(h1));
          $(item).prepend($(rdots));
...
function openPdfOption() {
  ...
  setTimeout(function () {
    ...
    $("#pdfPreview")[0].contentWindow.setSourceData(
      $("#source").val(),
      parseFloat($("#pdfScale").val()),
      topmargin,
      botmargin,
      leftmargin,
      rightmargin,
...
function openImgOption() {
  ...
  setTimeout(function () {
    ...
    $("#imgPreviewIframe")[0].contentWindow.setSourceData(
      $("#source").val(),
      parseFloat($("#imgScale").val()),
      topmargin,
      botmargin,
      leftmargin,
      rightmargin,
...
function setMargin() {
  ...
  $("#pdfPreview")[0].contentWindow.setMargin(
    topmargin,
    botmargin,
    leftmargin,
    rightmargin
...
function setImgMargin() {
  ...
  $("#imgPreviewIframe")[0].contentWindow.setMargin(
    topmargin,
    botmargin,
    leftmargin,
    rightmargin
...
function bindAllEvent() {
      ...
      if (selectText != "") {
        ...
        if ($(this).attr("id") == "nodecolorli") {
          ...
          val =
            '"' +
            val.replaceAll(/\"/g, "") +
            $("#nodecolorli").css("background-color") +
            '"';
        }
        ...
        if (position == "before") {
          ...
          if (selectText.indexOf("[") == 0) {
            replaceSelected(
              selectText,
              selectText.replaceAll("\\[", "").replaceAll("]", "")
            );
          } else {
            replaceSelected(
              selectText,
              "[" + selectText.replace(/\s/g, "") + "]"
            );
          }
          ...
          if (val == "!<(!") {
            ...
            if (!startExist) {
              newContent =
                content.substring(0, startSeq) +
                "!<(!" +
                content.substring(startSeq, endSeq) +
                "!<)!" +
                content.substring(endSeq);
            } else {
              newContent =
                content.substring(
                  0,
                  content.substring(0, startSeq).lastIndexOf("!<(!")
                ) +
                content.substring(startSeq, endSeq) +
                content.substring(endSeq).replace("!<)!", "");
            }
          } else if (val == "!>(!") {
            var startExist = false;
            var endExist = false;
            ...
            if (!startExist) {
              ...
              newContent =
                content.substring(
                  0,
                  content.substring(0, startSeq).lastIndexOf("!" + name + "!")
                ) +
                content.substring(startSeq, endSeq) +
                content.substring(endSeq).replace("!" + endName + "!", "");
            }
          }
          $("#source").val(newContent);
          abc_change();
          ...
          if (start_s.notes[0].a_dcn && start_s.notes[0].a_dcn.length > 0) {
            ...
            for (var i = 0; i < start_s.notes[0].a_dcn.length; i++) {
              ...
              if (name == "~(") {
                ...
                var preStr =
                  preStr.substring(0, si) +
                  preStr.substring(si + "!~(!".length);
  ...
  $(".operator_len")
    ...
    .on("click", function () {
      ...
      if (selectText != "") {
        ...
        var lastChar = $("#source")
          .val()
          .substr(getStartPos(getById("source")) - 1, 1);
        var chars = "123456789/";
        if (chars.indexOf(lastChar) > -1) {
          ...
          $("#source").val(
            $("#source")
              .val()
              .substr(0, getStartPos(getById("source")) - 1) +
              val +
              $("#source")
                .val()
                .substr(getStartPos(getById("source")))
          );
        } else {
          insertText(val);
        }
  ...
  $(".operator_sc")
    ...
    .on("click", function () {
      ...
      if ($(this).hasClass("jp_note")) {
        ...
        dot_selected_value = "";
        $(".dotstatus").removeClass("selected");
        setMicAquireNoteTime(); //如果有开启麦克风获取音符功能，则要设置当前标准音符时长
      }
      // 如果有选中编辑区的内容
      var selectText = getSelectText("source");
      ...
      if (selectText != "") {
        ...
        if (notes != null) {
          for (var i = 0; i < notes.length; i++) {
            ...
            temp = temp.replaceAll("/", "").replaceAll(/[0-9]/gi, "") + "1";
            result =
              result +
              temp.replaceAll(/\/+/gi, suffix).replaceAll(/[0-9]/gi, suffix);
          }
        }
  ...
  $("#chordCategory").on("change", function () {
    ...
    if (val == "3") {
      $(
        ".chord[value='zhuangwei3'],.chord_shuzi[value='zhuangwei3'],.chord_zimu[value='zhuangwei3']"
      ).css("display", "none");
    ...
    $(".chord,.chord_shuzi,.chord_zimu,.gongneng,.chord_f3").removeClass(
      "selected"
    );
    switchHx($("#K").val());
  });
  ...
  $("#chordLevel").on("change", function () {
    //		$("#chordType").find(".chord.selected").click();
    $(
      ".chord.selected,.chord_shuzi.selected,.chord_zimu.selected,.gongneng.selected,.chord_f3.selected"
    ).click();
    $(".chord,.chord_shuzi,.chord_zimu,.gongneng,.chord_f3").removeClass(
      "selected"
    );
  ...
  function _getChordVal() {
    ...
    var level =
      parseInt($("#chordLevel").find("option:selected").attr("level")) - 1; //级别 从I,II,III-VII
    ...
    var chord = $(
      ".chord.selected,.chord_zimu.selected,.chord_shuzi.selected,.chord_f3.selected"
    ).attr("value"); //类型：原位，第一转位、第二转位
    if (!chord) {
      return;
    }
    ...
    if (category == "3") {
      subtype = $(
        ".chord.selected,.chord_zimu.selected,.chord_shuzi.selected"
      ).attr("type3");
    } else if (category == "7") {
      subtype = $(
        ".chord.selected,.chord_zimu.selected,.chord_shuzi.selected"
  ...
  $(".chord,.chord_shuzi,.chord_zimu,.chord_gongneng,.chord_f3")
    .off("click")
    .on("click", function () {
      if ($(this).hasClass("selected")) {
        //已经选中，则取消选
      ...
      $(this)
        .parent()
        .find(".chord,.chord_shuzi,.chord_zimu,.chord_gongneng")
  ...
  $(".node_puhao")
    ...
    .on("click", function () {
      ...
      val = "[V: " + val + "]";
      var pattern =
        /\[V.*(alto)\]|\[V.*(bass)\]|\[V.*(treble)\]|\[V.*(tenor)\]/g;
      var selectText = getSelectText("source");
  ...
  $(".exportmp3")
    ...
    .on("click", function () {
      ...
      $.ajax({
        ...
        success: function (data) {
          $(".loading,.loading-box").remove();
          var mp3_down = document.createElementNS(
  ...
  $(".save_to_history")
    .off("click")
    .on("click", function () {
      $("#SERVER_HIS_div .modal-content").css(
        "left",
        ($(window).width() - $("#SERVER_HIS_div .modal-content").width()) / 2
      );
...
$(function () {
  ...
  // 键盘最小化
  $(".keyboard-bar .minimize")
    .off("click")
    .on("click", function () {
      $(".keyboard-box").animate({ height: "0px" }, 100);
      $("#scoreEditorPianoKeyboardContainer").animate(
        { bottom: -200 + "px" },
        100
      );
    ...
    success: function (data) {
      ...
      if (data != null && data != "") {
        ...
        for (var i = 0; i < rows.length; i++) {
          ...
          tr +=
            "<td onclick='getServerHisItemContent(\"" +
            row.STAFF_HISTORY_ID +
            "\")'>" +
            row.BACKUP +
            "</td>";
          tr +=
            ...
            "</textarea></td>";
          tr +=
            "<td><input type='button' value='删除' onclick='delServerHisItem(\"" +
            row.STAFF_HISTORY_ID +
            "\")'></td>";
          tr += "</tr>";
...
function switchHx(key) {
  ...
  //数字和弦
  if (shuzi_chord) {
    var shuZiYuanWei = shuzi_chord[chordCategory]["yuanwei"][jibie]
      .replace("(", '<span class="hx-sub">')
      .replace(")", "</span>");
    $(".hxspan_shuzi_yuanwei").html(shuZiYuanWei);
    var shuZiZhuanWei1 = shuzi_chord[chordCategory]["zhuangwei1"][jibie]
      .replace("(", '<span class="hx-sub">')
      .replace(")", "</span>");
    $(".hxspan_shuzi_zhuangwei1").html(shuZiZhuanWei1);
    var shuZiZhuanWei2 = shuzi_chord[chordCategory]["zhuangwei2"][jibie]
      .replace("(", '<span class="hx-sub">')
      .replace(")", "</span>");
    ...
    if (chordCategory == "3") {
      ...
      var shuZiZhuanWei3 = shuzi_chord[chordCategory]["zhuangwei3"][jibie]
        .replace("(", '<span class="hx-sub">')
        .replace(")", "</span>");
...
function changeSlurHeight(shValue) {
  ...
  if (s) {
    ...
    if (newContent == "") {
      newContent =
        content.substring(0, istart) + shStr + content.substring(istart);
    } else {
...
function reSelectSlur() {
  if (lastSelectedSlurIstart != -1) {
    ...
    $("path[type='slur'][istart='" + lastSelectedSlurIstart + "']").addClass(
      "selected_path"
    );
  }
}
...
function reSelectNote() {
  if (lastSelectedNoteIstart != -1) {
    $("text[type='hd'][istart='" + lastSelectedNoteIstart + "']").addClass(
      "selected_text"
    );
    $("text[type='Hd'][istart='" + lastSelectedNoteIstart + "']").addClass(
      "selected_text"
...
function localtosource() {
  ...
  } else if (!$("#editor").hasClass("menu-pressed")) {
    content_vue.editorClick();
  }
  ...
  findColsNumByIstart(ta, $("#source").val(), Number(istart));
}
//根据istart找到该音符在第几列
function findColsNumByIstart(ta, content, istart) {
  var lines = getLinesInfo(content);
  if (lines) {
    for (var i = 0; i < lines.length; i++) {
      var line = lines[i];
      if (line.startSeq <= istart && istart <= line.endSeq) {
        var dis = istart - line.startSeq;
        //console.log("width:"+$(ta).width(),"  scrollWidth：",ta.scrollWidth);
        var sw = ta.scrollWidth * (dis / (line.endSeq - line.startSeq));
        if (sw > $(ta).width()) {
          ta.scrollLeft = sw / 2;
        } else {
          ta.scrollLeft = 0;
...
function mNote(leftOrRight) {
  ...
  if (istart && istart != -1) {
    ...
    if (s) {
      ...
      if (newContent == "") {
        newContent =
          content.substring(0, istart) + xinStr + content.substring(istart);
      } else {
...
function switchMetro() {
  if (!$("#metronomesetting").hasClass("active")) {
    $("#metronomesetting").addClass("active");
    playMetro = true;
  } else {
    $("#metronomesetting").removeClass("active");
    playMetro = false;
  }
  src_change();
}
      ...
      var interval2 = setInterval(function () {
        if ($("#target>div>svg").length == $("input[name='staff1']").length) {
          ...
          var interval3 = setInterval(function () {
            if (
              $("#target>div>svg").length == $("input[name='staff2']").length
            ) {
...
function scOpen() {
  ...
  if (str == "视唱") {
    ...
    var speed = staffInfo.speed ? staffInfo.speed.val : 120;
    var beatTime = 2;
    if (staffInfo.speed) {
      beatTime =
        (60 / speed) *
        (staffInfo.speed.meter.bot / staffInfo.meter.bot) *
        staffInfo.meter.top;
    }
    user.delay.firstBeatNum = staffInfo.meter.top;
    user.delay.preNodeNum = staffInfo.meter.top < 4 ? 2 : 1;
    user.delay.onlyPreNode = 1;
    user.delay.time = beatTime * staffInfo.meter.top;
    playMetro = true;
    //src_change(function(){
    sc = new singVisual();
    sc.init();
    ...
    //					isSyncScore : false,
    //					isShowResult : true,
    //					musicScoreCball : function(res, params) {
    //						console.log(1)
    //					}
    //				}
    //
    //
    //			sc2 = new Sc($("#source").val(), scParam);
    //
    //			sc2.recordStart($("#source").val(), function() {
    //				play_tune(-1);
    //				$("#scspan").text("停止");
    //			});
    //});
  } else if (str == "停止") {
    user.delay.firstBeatNum = 0;
    user.delay.preNodeNum = 0;
    user.delay.onlyPreNode = 1;
    user.delay.time = 0;
    playMetro = false;
    stopSc();
...
//同步速度选择框内的中英文
function synSpeed(val) {
  $("#plsxspeedtype").val(val);
  $("#plsxspeedtype_en").val(val);
}
//初始化音色
function initInstrMenu() {
  ...
  $.contextMenu({
    ...
    callback: function (key, options) {
      ...
      content_vue.instrumentChoose(key);
    },
    items: items,
  });
}
...
function getMenuContent() {
  var arr = [
    { cname: "钢琴", cnos: ",0,1,2,3,4,5,6,7," },
    { cname: "风琴", cnos: ",16,17,18,19,20,21,22,136,23," },
    { cname: "弦乐", cnos: ",40,41,42,43,44,45,46," },
    { cname: "铜管", cnos: ",56,57,58,59,60,61,62,63," },
    { cname: "簧管", cnos: ",64,65,66,67,68,69,70,71," },
    { cname: "笛", cnos: ",72,73,74,75,76,77,78,79," },
    {
      cname: "打击乐",
      cnos: ",150,47,112,113,114,115,116,117,118,119,120,121,122,123,124,125,126,127,",
    },
    { cname: "色彩打击乐", cnos: ",8,9,10,11,12,13,14,15," },
    { cname: "吉他", cnos: ",24,25,26,27,28,29,30,31," },
    { cname: "贝司", cnos: ",32,33,34,35,36,37,38,39," },
    { cname: "世界", cnos: ",104,105,106,107,108,109,110,111," },
    {
      cname: "中国",
      cnos: ",15,131,132,133,135,137,138,139,140,141,149,165,166,167,168,169,170,171,172,",
    },
    { cname: "合奏合唱", cnos: ",48,49,50,51,55,52,53,54," },
    { cname: "合成人声", cnos: ",52,53,54,142,143,144,145,146,147,148,85," },
    { cname: "合成主音", cnos: ",85,80,81,82,83,84,86,87," },
    { cname: "合成音色", cnos: ",88,89,90,91,92,93,94,95," },
    { cname: "合成效果", cnos: ",96,97,98,99,100,101,102,103," },
    {
      cname: "奥尔夫",
      cnos: ",128,129,130,134,151,152,153,154,155,156,157,158,159,160,161,162,163,164,22,136,74,75,76,78,150,112,113,115,117,118,119,",
    },
  ];
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    group.name = o.cname;
    group.items = new Object();
    var cnos = o.cnos.split(",");
    for (var j = 0; j < cnos.length; j++) {
      var seq = cnos[j];
      if (seq != "") {
        group.items[seq + ""] = { name: content_vue.instrumentList[seq].name };
      }
    }
    items["group" + i] = group;
  }
