...
function Wijzer(xss, ymins, ymaxs, times, tixlb, lbtix, tixbts) {
  ...
  if (typeof lpRec == "undefined") {
    // skip if defined in preload
...
Wijzer.prototype.setline = function (line) {
  ...
  if (
    this.line_offsets[line + 1] > ymx ||
    this.line_offsets[line] < ntop + this.vmargin
  ) {
...
Wijzer.prototype.setx = function (x, xleft, xright) {
  // horizontal position in music line
  var n = $(".nobrk"),
    nleft = n.scrollLeft();
...
Wijzer.prototype.time2x = function (t, rondaf) {
  ...
  if (
    (opt.etime && t + offset > opt.etime) ||
    (!opt.etime && tix == times.length)
  ) {
    if (yubchk) {
  ...
  if (rondaf && times[tix] - t < 0.3) {
    // and times [tix] - t >= 0, by while loop above
  ...
  if (t <= 0 || t > lastTime)
    this.setx(0, 0, 0); // hide cursor if t not within score
...
Wijzer.prototype.doLoopTag = function (x, line, t, ix, tix) {
  function putTag(tag, x, line, next, mark, t, ix, tix) {
    if (!opt.lncsr) {
      // round click position to start or end of measure
      var xs = that.xs[line],
        ts = that.times;
      var xleft = xs[ix - 1],
        xright = xs[ix];
  ...
  switch (lpRec.loopBtn) {
    case 1:
      ...
      break;
    case 2:
      if (t > lpRec.loopStart)
        putTag("btag", x, line, 3, "loopEnd", t, ix, tix);
      break;
...
Wijzer.prototype.x2time = function (x, line) {
  ...
  if (opt.lopctl) {
    ...
    if (
      opt.synbox &&
      (yubchk
        ? elmed.getPlayerState() == YT.PlayerState.PLAYING
        : !elmed.paused)
    ) {
...
Wijzer.prototype.goMsre = function (b) {
  // one measure forwards (1) or backwards (0)
  var t,
    tix = this.time_ix;
  if (!elmed) return;
...
Wijzer.prototype.showSyncInfo = function () {
  ...
  $("#sync_info").html(
    "duration&nbsp;measure:<br>" + msre_dur.toFixed(3) + " sec.<br>"
  );
  $("#sync_info").append(
    "media&nbsp;offset:<br>" + offset.toFixed(3) + " sec."
  );
};
...
Wijzer.prototype.changeTimes = function (mnum, dt, dur) {
  // endtime += dt or endtime = begintime + dur
  var tix,
    tendnew,
    ts = this.times;
...
Wijzer.prototype.syncTimes = function (x, ix, line, tix) {
  // click location, measure index, line number, time index
  ...
  tleft = t;
  if (mnum == 0) {
    // click in first measure sets the offset
    offset += tleft; // approximated new begin time -> offset
    yubchk
      ? elmed.seekTo(offset + TOFF, true)
      : (elmed.currentTime = offset + TOFF); // jump to start
  ...
  if (tleft < tbeg + 0.5) {
    // 0.5 sec, beats per measure > 2 -> tempo > 240
    alert("tempo faster than 240 bpm: first sync previous measures");
    return;
  }
  if (this.lastSync > mnum) {
    // only change duration of mnum and keep the rest unchanged
    this.changeTimes(mnum, tleft - tend, 0);
  } else {
    // change duration of all measures >= mnum (extrapolate current tempo)
    this.changeTimes(mnum, 0, tleft - tbeg);
    this.lastSync = mnum;
  }
  if (opt.jump) {
    yubchk
      ? elmed.seekTo(tbeg + offset + TOFF, true)
      : (elmed.currentTime = tbeg + offset + TOFF); // jump to start of previous measure
  }
};
...
function toggleScoreBtn() {
  ...
  b.html(
    c
      ? h.replace("score file", "<b>import</b>")
      : h.replace("<b>import</b>", "score file")
  );
...
function doDrop(e) {
  ...
  if (/video|audio/.test(files[0].type))
    // xml => 'text/xml', abc => ''
    readMedia("dd", files);
...
function readMediaYub() {
  if (!$("#yubid")[0].checkValidity()) {
    alert(
      "The youtube video id should be 11 characters long,\neach from 'A' to 'Z', 'a' to 'z', '0' to '9', '-' or '_'"
    );
    return;
  }
...
function initPbRates(min, max, inc) {
  pbrates = [];
  for (var x = min; x <= max + 0.001; x += inc) {
    // 0.001 because error in javascript integers !!
...
function yubApiReady() {
  ...
  ybplayer = new YT.Player("vidyub", {
    events: { onReady: klaar, onStateChange: toestand },
  });
}
...
function setPlayer(fnm, mediaSrc) {
  ...
  if (!fnm) {
    ...
    elmed = $elmed.get(0);
    if (/\.ogg$/i.test(mediaSrc)) {
      // may be also mp3 file present
      if (!elmed.canPlayType("audio/ogg"))
        mediaSrc = mediaSrc.replace(/\.ogg$/i, ".mp3");
    }
    if (/\.webm$/i.test(mediaSrc)) {
      if (!elmed.canPlayType("video/webm"))
        mediaSrc = mediaSrc.replace(/\.webm$/i, ".mp4");
    }
...
function centerPlayer() {
  ...
  $("#vidyub").css(
    "margin-left",
    opt.ctrmed ? (ww - wy).toFixed() + "px" : "0px"
  );
...
function dolayout(abctxt) {
  ...
  function img_out(str) {
    if (str.indexOf("<svg") != -1) {
      str = str.replace(
        /width="(\d*)px"\s*height="(\d*)px"/,
        'width="$1px" height="$2px" viewbox="0 0 $1 $2"'
      );
      (bxs = keySort(bxs)), (bys = keySort(bys));
      if (
        bxs.length > 1 && // the first barline is at bxs[1] because bxs[0] == left side staff
  ...
  function compPlayMap() {
    ...
    while (1) {
      ...
      if (!volta || volta >= repnum) {
        // skip when repnum > volta num
        pbtime += mdurs[mix];
        times.push(pbtime);
      ...
      if (r && repnum == 1 && !repdone[mix] && !opt.repskip) {
  ...
  msc_svgs.each(function () {
    // take the <svg> element for drawing, but
  ...
  if (typeof times_arr != "undefined") {
    // external timings take precedence.
...
function flattenTimes(times) {
  // the number of measure per line may have changed since synchronization.
...
function tick() {
  if (!elmed) return;
  var t = (yubchk ? elmed.getCurrentTime() : elmed.currentTime) - offset,
    tcur = t;
  if (t < 0) t = 0;
  if (opt.lopctl) {
    ...
    if (t != tcur)
      yubchk
        ? elmed.seekTo(t + offset, true)
        : (elmed.currentTime = t + offset);
  }
...
function btnChk() {
  ...
  if (opt.btns && typeof FileReader == "undefined") {
    $(".nobrk").prepend(
      "<h3>Your browser does not support reading of local files ...</h3>but you can use the preload feature."
    );
  }
}
...
function clear_metronome() {
  if (!msc_wz) return;
  setTimeout(function () {
    // use the event queue to avoid race condition with new telaf cycle
    clearInterval(in_count_in);
    in_count_in = 0;
    msc_wz.tiktak.text("");
  }, 0);
...
function playPause(cmd, delay) {
  // cmd = do_pause : at_time
  if (!elmed) return;
  var x = cmd.split(":"),
    toggle = x[0] == "true",
    time = parseFloat(x[1]),
  ...
  yubchk
    ? yubstate != 5 && elmed.seekTo(time, true)
    : (elmed.currentTime = time); // always position (except when cued)
  if ((paused && toggle) || (!paused && !toggle)) {
    // play
    if (fcount_in) {
      // play after count in
...
function playPause2(toggle, t) {
  // toggle player state : go to t : with count in, only user actions come here
  var cmd =
    toggle + ":" + t.toFixed(2) + ":" + (toggle && $("#cntin").prop("checked"));
  if (sok) sok.send(cmd);
  else playPause(cmd, 0); // synchronize when socket connection
}
...
function hideMenuHelp() {
  // called from click event dispatcher -> the change event could come later
  var b =
    $("#menu label").css("display") != "none" || $("#help").hasClass("showhlp");
  if (b) {
