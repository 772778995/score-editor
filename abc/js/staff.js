...
staff.prototype = {
  ...
  run: function () {
    ...
    if (currIndex < this.arr.length) {
      ...
      w =
        (this.arr[currIndex].barline_end[0] -
          this.arr[currIndex].barline_start[0]) *
        persent;
      h =
        (this.arr[currIndex].barline_start[3] -
          this.arr[currIndex].barline_start[1]) *
        persent;
      if (w < 0) {
        currIndex++;
        this.run(cxt);
  ...
  drawCycle: function (num) {
    console.log("num:" + num);
    this.cxt2.beginPath();
    this.cxt2.fillStyle = "#0E518F"; //填充颜色,默认是黑色
    var startx = 20;
