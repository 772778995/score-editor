...
function updownnote(oldnotestr,interval){
	...
	if(oldnotestr.replace(/\".[^\"]*\"/g,"").indexOf("[")>-1 && oldnotestr.replace(/\".[^\"]*\"/g,"").indexOf("]")>-1){
		var p = /\[(.*)\]\d*/g;
...
//					console.log("----------nodeStr:",nodeStr)
					...
					if(nodeStr.replace(/\s+/g,"")!="" && (/[a-gA-G]/.test(nodeStr) || nodeStr.indexOf("m")>-1 || nodeStr.indexOf("h")>-1 || nodeStr.indexOf("n")>-1 || nodeStr.indexOf("z")>-1 || (staffInfo.vocalCount>1 && nodeStr.indexOf("x")>-1))){
						...
						//如果只有x和数字 
						if(!/[a-gA-g]/.test(tmpStr.replace(/\|\]/,"")) && tmpStr.indexOf("x")>-1){
							//如果只有一个声部
...
function checkBarIsInLineFirst(barRect){
	...
	if(musicType==2){
		findStr = "rect[type='splnum_note'],rect[type='splnum_rest'],rect[type='splnum_chord']";
	}
...
//		$("#layoutPanel").removeClass("active")
		...
		if(!content_vue.attrPanelShow){
			...
			setTimeout(function(){
				$("text[gch_istart="+gch_istart+"]").attr("selected","selected").css("color","red")
...
function getNodeWidth(){
	...
	$("#barWidthInput").val(width);
	$("#barWidthInput").attr("oldWidth",width);
	$("#barWidthInput").val(1.00);
	$("#barWidthInput2").val(1.00);
}
//获取小节高度
					...
					if(matchs!=null){
						for(var k=0;k<matchs.length;k++){
							...
							if(nodeStr.indexOf(botStr)>-1){
								//下高度
								$("#barHeightBotInput").val(matchs[k].replace(reg2,function(s1,s2){
										return s2.trim();
									}))
							}else{
								//上高度
								$("#barHeightTopInput").val(matchs[k].replace(reg2,function(s1,s2){
									return s2.trim();
								}))
							}
						}
...
function genChangMingAbc(content){
		...
		for(var i=0;i<ae.length;i++){
			...
			if(s){
				...
				if(breathIstart == s.istart){
					...
					var tmpNoteStr = noteStr.replace(/[\{\}\(\)]/g,"").replace(/\d/g,"").replace(/[/]/g,"");
				...
				if(s.type==8){
					if(s.slur_istart){
						...
						if(reg.test(oldNoteStr)){
							...
							var num = getSimpleNameByKAndStaff(keyTransfer[kv].value,oldNoteStr.replaceAll(/[\/0-9\(\)\-]/,""),content);
	...
	var result = preContent + staffContent.replaceAll(/[mhn]/g,"B") + "\n" + lyricContent + "\n";
	console.log(result);
	return result;
}
...
function sendChangMingContent2Server(content,info,groupId,abcUrl){
    ...
    $.ajax({
        ...
        type:'POST',
        async: true,
        data: {"abcContent":content,"info":info,"groupId":groupId,"pytbData":JSON.stringify(pytbData)},
...
//							note.currlyric = "";
	...
	jsonDataObj.speed = staff.speed.meter.top+"/"+staff.speed.meter.bot+"="+staff.speed.val;
...
function saveasjson(){
	...
	obj.speed = staff.speed.meter.top+"/"+staff.speed.meter.bot+"="+staff.speed.val;
	obj.data = getStandJsonData();
	var pval = "";
	var s = srcidx == 0 ? "source" : "src1", source = JSON.stringify(obj), uriContent = "data:text/plain;charset=utf-8," + encodeURIComponent(source), link = document.createElement("a");
...
function getDurTimeBySpeed(speed,dur){
	var speedmap = {"1/64":24,"1/32":48,"1/16":96,"1/8":192,"1/4":384,"1/2":768,"1/1":1536}
