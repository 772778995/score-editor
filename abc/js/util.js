...
String.prototype.replaceAllNum = function (s1) {
  return this.replace(new RegExp(/[0-9]/gi, "gm"), s1);
};
...
function getStartPos(textarea) {
  if (typeof textarea.selectionStart != "undefined") {
    ...
    for (
      var sel_start = 0;
      range_textarea.compareEndPoints("StartToStart", range) < 0;
      sel_start++
    )
...
function insertText(str) {
  ...
  if (document.selection) {
    ...
    obj.value =
      tmpStr.substring(0, cursorPos) +
      str +
      tmpStr.substring(cursorPos, tmpStr.length);
...
function maskFullTxt(txt) {
  ...
  html +=
    '<div class="modal center-modal fade show loading-box" id="modal-mask" ondblclick="hideMaskFullTxt()">';
...
function getNodeDatas_v2(source) {
  ...
  for (var i = 1; i <= size; i++) {
    ...
    if (i > 1) {
      node_contents.push(
        content.substring(
          content.indexOf(last_pattern_str) + last_pattern_str.length,
          content.indexOf(pattern_str)
        )
      );
    }
  ...
  node_contents.push(
    content.substr(content.indexOf(pattern_str) + pattern_str.length)
  );
...
function updownnote(oldnotestr, interval) {
  ...
  if (
    oldnotestr.replace(/\".[^\"]*\"/g, "").indexOf("[") > -1 &&
    oldnotestr.replace(/\".[^\"]*\"/g, "").indexOf("]") > -1
  ) {
    var p = /\[(.*)\]\d*/g;
  ...
  while ((notestr = pattern.exec(substr))) {
    ...
    if (o.old_note == "z") {
      ...
      if (Math.abs(interval) == 1) {
        ...
        o.new_note = getStandNoteByInterval(
          notestr[0].replace(/[\^\_\=]/g, ""),
          interval
        );
        if (preStr != "") {
          //把前面的升降号替换为原来的
...
function getLyricDiv_nouse(sourceid) {
  ...
  var lines = content.split("\n");
  var input_demo =
    '<input type="text" style="width:40px;margin:1px;" line="var_line" index="var_index" note="var_note" value="var_value" inner_line="0">';
  // 音符正则表达式
  var notes_pattern =
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (
        ...
        if (line.indexOf("w:") < 0 && line.indexOf("N:") < 0) {
          ...
          if (notes != null) {
            ...
            for (var k = 0; k < notes.length; k++) {
              if (note_pattern.test(notes[k])) {
                inputbuffer += input_demo
                  .replace("var_index", input_count++)
                  .replace("var_note", notes[k])
                  .replace("var_value", "value_" + note_index)
                  .replace("var_line", line_count);
                line_demo += input_demo
                  .replace("var_index", input_count++)
                  .replace("var_note", notes[k])
                  .replace("var_value", "value_" + note_index)
                  .replace("var_line", line_count);
          ...
          inputbuffer +=
            '<input type="button" value="+" onclick="add_lyric_line(\'' +
            sourceid +
            "'," +
            line_count +
            ')">';
...
function add_lyric_line(sourceid, line_num) {
  ...
  $.each(
    $("#lyric_edit_div input[line='" + line_num + "'][inner_line='0']"),
    function (i, item) {
      var c = $(item).clone();
      $(c)
        .attr("inner_line", max_inner_line + 1)
        .val("");
...
function getMaxInnerLine(line_num) {
  var max = 0;
  $.each(
    $("#lyric_edit_div input[line='" + line_num + "']"),
    function (i, item) {
...
function getPartStaff(sourceid, start, end, stave) {
  ...
  if (stave) {
    ...
    for (var i = 0; i < slist.length; i++) {
      ...
      if (
        slist[i].istart == start ||
        getLyricIstart(slist[i]).indexOf(start) > -1
      ) {
        startBar = slist[i].bar_index;
      }
      if (
        slist[i].istart == end ||
        getLyricIstart(slist[i]).indexOf(end) > -1
      ) {
        ...
        if (result != null) {
          ...
          for (var k = 1; k < 10; k++) {
            if (k + i < lines.length) {
              ...
              var line_lyric = getLyricByLineStr(line_str);
...
// 根据给定的一行字符串，解析出歌词
function getLyricByLineStr(line_str) {
  ...
  if (words != null) {
    ...
    for (var i = 0; i < words.length; i++) {
      if ($.trim(words[i]) != "") {
        ...
        preStr += sufStr.substring(
          0,
          sufStr.indexOf(words[i]) + words[i].length
        );
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (index > startPos) {
      ...
      if (prefix_str.indexOf("w:") > -1) {
        ...
        var tmpstr = prefix_str
          .split("|")
          [obj.nodeIndex].replace("w:", "")
    ...
    if (linedata.line_index == obj.line_index) {
      ...
      for (var j = 0; j < linenotes.length; j++) {
        ...
        if (hasNodeLineInLyric) {
          if (
            linenotes[j].noteInNodeIndex == obj.note_index &&
            linenotes[j].nodeIndex == obj.nodeIndex
          ) {
            ...
            if (line_lyric.length < line_notes.length) {
              for (var k = start_index; k < line_notes.length; k++) {
                ...
                for (var m = 0; m < line_lyric.length; m++) {
                  ...
                  if (
                    lyric.nodeIndex == note.nodeIndex &&
                    lyric.noteInNodeIndex == note.noteInNodeIndex
                  ) {
...
function convert2Rhythm(sourceid) {
  ...
  for (var i = 0; i < lines.length; i++) {
    var line = lines[i];
    if (
      line.replace(/\{.[^\}]*\}/g, "").indexOf("|") > -1 &&
      line.replaceAll(" ", "").indexOf("w:") != 0 &&
      line.indexOf("N:") != 0
    ) {
      first_note_line = i;
...
function findPosition(evt, svg) {
  ...
  $.each($(svg).find("rect.abcr"), function (i, item) {
    ...
    // ((parseFloat(rect.y)+translateY)*scale+parseFloat(rect.height)*scale)),2));
    var distance4 =
      Math.abs(x - (rect.x + rect.width)) +
      Math.abs(y - (rect.y + rect.height));
...
function addCursor(evt, rect, rectArr) {
  ...
  if (rect.pos == "behind") {
    ...
    setCaretPosition(
      ...
      rect.index + tmpNote.replaceAll(" ", "").length
    );
  } else {
    // 光标是最近的音符前面
  ...
  var divStr =
    '<div id="mycursor" style="position:absolute;left:' +
    (evt.pageX - x) +
    "px;top:" +
    (rect.staffTop - y) +
...
function genRhythm(sourceid, preNodeNum, type) {
  ...
  var rhythmNodeContentTitle =
    "V:99 perc stafflines=1\n%%MIDI program 115\nK:C shift=cc octave=0\n";
...
function addPreNodeAndRhythm(sourceid, preNodeNum, type, standV) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (
      lineRhythm != "" &&
      (line.indexOf("%%score") > -1 || line.indexOf("%%staves") > -1)
    ) {
      //这段逻辑应对开始有共公声部，后面分声部的情况
      staff +=
        "V:99 perc stafflines=1\n%%MIDI program 115\nK:C shift=cc octave=0\n" +
        lineRhythm;
      lineRhythm = "";
    ...
    if (lineObj.type == "note") {
        ...
        for (var k = 0; k < lineNodes.length; k++) {
          ...
          if (
            ...
            (/[a-gA-G]/.test(nodeStr) ||
              ...
              nodeStr.indexOf("n") > -1 ||
              nodeStr.indexOf("z") > -1 ||
              (staffInfo.vocalCount > 1 && nodeStr.indexOf("x") > -1))
          ) {
            if (/\[M:.*\]/.test(nodeStr)) {
              lineRhythm += /\[M:.[^\[]*\]/.exec(nodeStr)[0];
            }
            var tmpStr = nodeStr
              .replaceAll(/\[\".[^\"]*\"/g, "")
              .replaceAll(/\[.\:.[^\]]*\]/g, "")
            ...
            if (nodeStr.indexOf("x") > -1) {
              ...
              nodeLen = calNodeLenByIndex(
                lineNodes[k].startSeq,
                lineNodes[k].endSeq
              );
            }
            ...
            if (nodeLen <= n && nodeLen > 0) {
              ...
              if (nodeLen > 0) {
                ...
                if (nodeLen > 1 && parseInt(nodeLen) != nodeLen) {
                  ...
                  nodeLen > 1 &&
                  parseInt(nodeLen) == parseInt(nodeLen)
                ) {
                  lineRhythm += c + parseInt(nodeLen);
                  nodeLen = 0;
                }
      ...
      if (firstLine == 0) {
        ...
        if (misLen > 0) {
          ...
          line =
            line.substr(0, startIndex) + bStr + sStr + line.substr(startIndex);
        }
        // 添加预备拍
        if (preNodeNum != 0) {
          for (var x = 0; x < preNodeNum; x++) {
...
function getBarLineData() {
  ...
  $.each($("svg.music"), function (i, item) {
    ...
    $.each(
      $(item).find("rect[type='bar'],rect[type='splnum_bar']"),
      function (j, rect) {
        var seq = $(rect)
          .attr("class")
          .match(/\_([0-9]*)\_/)[1];
...
//显示/隐藏小节线序号
function showMeasurebox(sourceid) {
  var content = $("#" + sourceid).val();

  if (content.indexOf("%%measurebox") > -1) {
    content = content.replaceAll(/%%measurebox\n/, "");
  } else {
    content = "%%measurebox\n" + content;
  }
  $("#" + sourceid).val(content);
  ...
  for (var j = 0; j < varr.length; j++) {
    ...
    for (var i = 0; i < syms.length; i++) {
      ...
      if (prog.indexOf("PRONUM") > -1) {
        prog = prog.replace(
          "PRONUM",
          s.p_v.instr == undefined ? 0 : s.p_v.instr
        );
      }
      var lineNum = findLineNumByIndex(content, s.istart);
      if (lineNum != lastLineNum) {
        if (
          lines[lineNum].indexOf("V:") < 0 &&
          lines[lineNum].indexOf("score") < 0 &&
          lines[lineNum].indexOf("stave") < 0
        ) {
    ...
    if (matchs != null) {
      ...
      if (numReg.test(fontStr)) {
        content = content.replace(
          fontStr,
          fontStr.replace(/[0-9]\d*/, fontSize)
        );
      } else {
...
function showStrongWeak(sourceid) {
  ...
  if (switchMeter.length > 0) {
    ...
    $("#METER_MODEL_div .modal-content").css(
      "left",
      ($(window).width() - $("#METER_MODEL_div .modal-content").width()) / 2
    );
...
function convert2RhythmAndPlay(content, varr, score) {
  ...
  var noScore =
    content.indexOf("%%score") == -1 && content.indexOf("%%staves") == -1;
  var isSkip = false;
  var start = 0;
  var end = 0;
  var count = 0;
  ...
  if (staff.match(/L:/g) != null && staff.match(/L:/g).length > 1) {
    staff = staff.replace(
      /V:9 perc stafflines=1 octave=0/g,
      "V:9 perc stafflines=1 octave=0\nL:" + L + "\n%%MIDI program 115"
    );
  }
  return staff;
...
function renderMusicFormByNote(mfSetting, scale, isSelNote) {
  if (mfSetting != null && mfSetting.length > 0) {
    ...
    if (musicType == 2) {
      ...
      $notes = $(
        "svg rect[type='splnum_note']:not(.ph), svg rect[type='splnum_rest']:not(.ph)"
      );
    } else {
    ...
    $notes.each(function (index, item) {
      ...
      var barPo = barlineArr.find(function (item) {
        ...
        return (
          item.line == line &&
          item.barline_start[0] <= fx &&
          item.barline_end[0] >= fx
        );
      });
      if (barPo) {
        offsetArr.push({
    ...
    for (var i = 0; i < offsetArr.length; i++) {
      ...
      if (
        ...
        noteS.prev &&
        noteS.prev.type == 0 &&
        syms[noteS.istart].my_line == syms[noteS.prev.istart].my_line
      ) {
      ...
      if (sLine == eLine) {
        ...
        for (var j = mf.endNodeIndex - 1; j >= mf.startNodeIndex; j--) {
          ...
          if (oLine != eLine) {
            ...
            if (oLine != sLine) {
              // 说明这是跨多行，前面还有跨行情况，继续循环
              isLastLine = false;
              eTop = oTop;
              eLeft = barlineArr[j].barline_end[0];
            } else if (oLine == sLine) {
              // 当前已经是第一行了，前面没有跨行了，开始渲染第一行
...
function draw_slur() {
  var path =
    '<path class="mypath stroke" d="M48.93 197.00 C100 197 130 197 152 119"></path>';
}
...
function appendDivstr(mf, line, top, left, width, level) {
  ...
  //				 '</div>';
  var divstr =
    '<div class="music-form _id" data-id="_kid" onclick="mfClick(event)" style="position:absolute;overflow:hidden;font-size:18px;color:#fff;text-align:center;line-height:30px;height:30px;top:_top;left:_left;width:_width;">' +
  ...
  var ds = divstr
    .replace("_top", top + "px")
    .replace("_width", width + "px")
    .replace("_left", left + "px")
    .replace("_bgColor", color);
  if (mf.fieldno) {
  ...
  ds = ds
    .replace("_title", mf.title)
    .replace("_desc", mf.desc)
    .replace(
      "_id",
      "mf-" +
        mf.startNodeIndex +
...
function setNoteBgColor(type) {
  ...
  if (selectedNotes.length == 1) {
    ...
    content =
      content.substring(0, istart) + startStr + content.substring(istart);
    var s = syms[istart];
    if (s) {
    ...
    content =
      content.substring(0, sistart) +
      startStr +
      content.substring(sistart, eistart) +
      endStr +
      content.substring(eistart);
    var ss = syms[sistart];
    var es = syms[eistart];
    if (ss) {
  ...
  for (var i = 0; i < noteBgArray.length; i++) {
    ...
    if (start && end) {
      ...
      if (isLongRender) {
        var next = e.next;
        while (next) {
          if (next.grace || (next.extra && next.extra.grace)) {
            //在后面是倚音的情况下，继续往后寻找
            next = next.next;
          } else {
            break;
      ...
      $(start_rect)
        ...
        .prepend(
          '<svg id="note_bg_svg' +
            start.seq +
            '" type="note_rect"><rect type="bg_rect" x="' +
            (start_x + trans_x) * scale +
            '" y="' +
            (start_y + trans_y) * scale +
            '" width="' +
            w * scale +
            '" height="' +
            h * scale +
    ...
    if (start && end) {
      ...
      $(start_rect)
        ...
        .prepend(
          '<svg id="lyric_bg_svg' +
            start.seq +
            '" type="lyric_bg_rect"><rect type="bg_rect" x="' +
            (start_x + trans_x) * scale +
            '" y="' +
            (start_y + trans_y) * scale +
            '" width="' +
...
function setArpLink(source) {
  ...
  var preStr = $("#" + source)
    .val()
    .substring(startPos - 19, startPos);
  // 去掉开始
  ...
  if (ts_next && ts_next.st != cen.st) {
    ...
    if (cen.istart < tsNext.istart) {
      ...
      newContent =
        ...
        startStr +
        content.substring(cen.istart, tsNext.istart) +
        endStr +
        content.substring(tsNext.istart);
    } else {
      //选中的是下面声部
      var tsPrev = cen.ts_prev;
      if (tsPrev && tsPrev.st != cen.st) {
        newContent =
          content.substring(0, tsPrev.istart) +
          startStr +
          content.substring(tsPrev.istart, cen.istart) +
          endStr +
          content.substring(cen.istart);
      }
    }
    if (newContent != "") {
...
function calNodeLen(nodestr) {
  try {
    ...
    len =
      parseFloat(hx_len_val) + parseFloat(ly_len_val) + parseFloat(note_len);
    return len;
  } catch (e) {
    console.log(e);
  }
...
//重置每行小节数，在一屏显示后，会导致有些行很宽松，有些行很拥齐，调用该方法让每行的小节数平均一下
function resetBarsPerLine() {
  //	%%barsperstaff 4
  var svgNums = $("svg").length;
  var si = getStaffInfo("source");
  var barNumsPerLine = Math.ceil(si.barCount / svgNums);
  var content = $("#source").val();
  content.replace(/%%barsperstaff.*\n/, "");
  content = "%%barsperstaff " + barNumsPerLine + "\n" + content;
  $("#source").val(content);
}
...
function numStaffDrag(e) {
  ...
  if (e.offsetY - lastNumStaffY > 10) {
    ...
    var newContent =
      content.substring(0, s.istart) + newStr + content.substring(s.iend);
    $("#source").val(newContent);
    doLog();
    src_change();
    lastNumStaffY = e.offsetY;
...
function getLinesInfo(content) {
  ...
  for (var i = 0, len = lines.length; i < len; i++) {
    ...
    if (scoreReg.test(tmpLineStr)) {
      ...
      tmpLineStr.indexOf("|") > -1 ||
      tmpLineStr.indexOf("S:") == 0 ||
      tmpLineStr.indexOf("w:") == 0
    ) {
      if (nextV == -1) {
        nextV = 0;
      }
...
function getNodesInfo(content) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (
      startFlag &&
      (type == "note" || type == "w" || type == "N" || type == "S")
    ) {
      ...
      while ((node = reg.exec(lineStr))) {
        ...
        nodeObj.nodeIndex =
          globalNodeIndexMap.get("globalNodeIndexMap" + v) + nodeIndex++;
        if (lastVLineIndex == vLineIndex && line.v == lastV) {
...
function scroll2SelectNote(istart) {
  ...
  ta.scrollTop =
    (line * ta.scrollHeight) / $("#source").val().split("\n").length - 10;
  var s = syms[istart];
  if (s) {
...
function toStaff() {
  ...
  for (var i = 0, len = syms.length; i < len; i++) {
    ...
    if (c) {
      ...
      console.log(
        "str:",
        content.substring(c.istart, c.iend),
        "  istatt-iend:",
        c.istart,
        c.iend,
        "  st:",
    ...
    for (var i = 0, len = syms.length; i < len; i++) {
      ...
      if (c) {
        if (
          ...
          if (i == c.istart) {
            //这里加这个判断是因为如果音符前面有注释，也会生成一个同样的istart的syms对象，不知道会不会带来其它问题
            if (match == 0) {
              noteInNodeSeq++;
            }
            totalTime += c.dur;
              ...
              if (cb.barNum == nodeCount) {
                if (cb.v != -1) {
                  if (cb.v == lineObj.v) {
                    ...
                    copyNodeInfo.set(
                      "v" + cb.v,
                      copyNodeInfo.get("v" + cb.v) +
                        lineObj.lineStr.substring(lastIndex, node.index) +
                        node[0]
                    );
                  }
                } else {
          ...
          for (var j = 0; j < pasteBarInfoArr.length; j++) {
            ...
            if (pb.barNum == node.nodeIndex && pb.v == lineObj.v) {
              ...
              if (!copyNodeStr && copyVCount == 1) {
                copyNodeStr = copyNodeInfo.get(
                  copyNodeInfo.keys().next().value
                );
              }
              if (!hasPaste) {
...
//复制音符,支持单个音符，多个音符复制，不支持跨小节复制，如果需要复制小节，可选中整个小节进行复制
function copyNote() {
  hiddenMenu();
  ...
  if (selectNotes.length > 0) {
    ...
    } else if (selectNotes.length > 1) {
      var sIstart = $(selectNotes[0]).attr("istart");
      var eIstart = $(selectNotes[selectNotes.length - 1]).attr("istart");
      var ends = syms[eIstart];
      for (var i = sIstart; i <= eIstart; i++) {
        var s = syms[i];
        if (s && (s.type == 8 || s.type == 10)) {
          user.copyNoteInfo.dur += s.dur_orig;
        }
      }
      if (ends) {
        var content = $("#source").val();
        var noteStr = content.substring(sIstart, ends.iend);
        user.copyNoteInfo.copyNoteStr = noteStr;
      }
    }
  }
  ...
  if (selectNotes.length == 1) {
    ...
    if (s) {
      ...
      if (user.copyNoteInfo.copyNoteStr != "") {
        ...
        noteInfo.note = user.copyNoteInfo.copyNoteStr
          .replace(/\d/g, "")
          .replace(/[\/]/g, "");
    ...
    var s = syms[istart];
    if (s) {
      if (user.copyNoteInfo.dur > s.dur) {
        window.top.alert(
          "当前复制的音符时值大于选中的音符时间，无法完成粘贴操作。"
        );
        return;
      }
      var content = $("#source").val();
...
function durToTime(dur, q) {
  if (dur > 0 && q) {
    ...
    var qSecond = 60 / q.split("=")[1]; // 计算一个音符是多少秒
        ...
        if (staff.lineIndex == line) {
          ...
          if (addTop) {
            if (musicType == 2) {
              ...
              ymin =
                (parseFloat(staff.translate.y) - 24) * scale + data.tops[line];
...
function checkBarIsInLineFirst(barRect) {
  ...
  $(svg)
    ...
    .each(function (i, item) {
      ...
      if (ix < barX) {
        //有比小节线的x坐票更小的音符或休止符，则说明不是以小节线开头
        isInLineFirst = false;
      }
    });
...
function renderStaffNodeBySt(indexArr, typestr) {
  ...
  for (var i = 0; i < indexArr.length; i++) {
    ...
    for (var j = 0; j < data.length; j++) {
      var currData = data[j];
      if (
        currData.v == indexArr[i].v &&
        currData.node_index == indexArr[i].bar_num
      ) {
    ...
    if (parseInt(n) <= 4) {
      if (nodeLen % z == 0) {
        if (lineIndex != lines.length - 1) {
          //不是最后一行时，增加一个回车符
          $("#source").val(
            preContent.substring(0, preContent.length - 1) +
              " \n" +
...
function delNodes(nodeIndexArr) {
  ...
  result = replaceBlankLine(result);
  //以下判断如果删除后第一行是w:，则删除
  var lines2 = getNodesInfo(result);
  var noteOrLyricCount = 0;
  var newResult = "";
  for (var i = 0; i < lines2.length; i++) {
    var line = lines2[i];
    var lineStr = line.lineStr;
    if (line.type == "note" || line.type == "w") {
      noteOrLyricCount++;
    }
    if (noteOrLyricCount == 1 && line.type == "w") {
    } else {
      newResult += lineStr + "\n";
    }
  }
  $("#source").val(newResult);
  src_change();
  doLog();
}
//根据小节序号插入小节
...
function resetMeter(meter) {
        ...
        if (s.type == 8 || s.type == 10) {
          ...
          if (totalDur == nodeDurVal) {
            ...
            if (moreDurVal < nodeDurVal) {
              ...
              var currNoteDurStr = getDurStrByNoteDur(
                currNoteRemainDur,
                s.my_ulen
              ); //当前音符被切割的前半部分时值字符串
  ...
  result = mergeRest(
    result,
    (toFloat(meter) / toFloat(oldMeter)) * nodeTotalLen
  ); //合并同一小节内的休止符
...
function showProperties(type, e, clickObj) {
  ...
  if (type == "lyric") {
    ...
    } else if (upFlagReg.test(textStr) || downFlagReg.test(textStr)) {
      //上标
      if (upFlagReg.test(textStr)) {
        var node = upFlagReg.exec(textStr);
        upText = node[1];
        textStr = textStr.replace(node[0], "");
      }
      //下标
      if (downFlagReg.test(textStr)) {
        var node = downFlagReg.exec(textStr);
        downText = node[1];
        textStr = textStr.replace(node[0], "");
      }
      text = textStr;
    ...
    //		$("#layoutPanel").removeClass("active")
  } else if (type == "text") {
    $("#panelName").html("文本属性");
    ...
    if (s) {
      ...
      getNoteBgColor(s);
      //获取选中音符的简谱文本
      var noteSplText = getNoteSplText(s);
      var noteSplTextMidi = getNoteSplTextMidi(s);
      $("#noteJpText").val(noteSplText);
      $("#noteJpTextMidi").val(noteSplTextMidi);
      if (s.my_key) {
    ...
    if (!istart) {
      istart = $(selectDecoInfo).attr("start");
    }
    var s = syms[istart];
    var fermata = getA_ddInfo(s, $(selectDecoInfo).attr("type"));
    if (fermata.length > 0) {
      var fermatatime = s.fermatatime;
      if (!fermatatime) {
        fermatatime = 1;
      }
      $("#fermatatimeInput").val(fermatatime);
      $("#fermatatimediv").show();
    } else {
      $("#fermatatimediv").hide();
    }
    ...
    if (s) {
      ...
      if (gch) {
        for (var i = 0; i < gch.length; i++) {
          if (gch[i].text.replace(/[_^]/, "") == gchText.replace(/[_^]/, "")) {
            pos = gch[i].type;
      ...
      $("input[name='gchdirect'][value='" + pos + "']").prop(
        "checked",
        "checked"
      );
      if (fontType == "∆") {
        $("input[name='gchfonttype'][value='∆']").each(function (i, item) {
...
function getNodeWidth() {
    ...
    if (line.type == "note") {
      ...
      for (var j = 0; j < nodes.length; j++) {
        ...
        if (node.nodeIndex == index) {
          var nodeStr = node.nodeStr;
          //"barwidth:"
          var reg = /\"barwidth\:([0-9]{1,})\"/;
          var match = nodeStr.match(reg);
          if (match != null) {
            width = match[1];
            $("#barWidthInput").val(width);
            $("#barWidthInput").attr("oldWidth", width);
  ...
  if (width == "") {
    if (selectedBars.length > 0) {
      width = parseInt(selectedBars[0].getBBox().width / scale);
    }
  }
  console.log("左边距", $(selectedBars[0]).find("rect").attr("x"));
  var leftMargin = parseInt($(selectedBars[0]).find("rect").attr("x"));
  if (leftMargin < 50) {
    width = width - 58 * scale;
  }
  $("#barWidthInput").val(width);
  $("#barWidthInput").attr("oldWidth", width);
  $("#barWidthInput").val(1.0);
    ...
    if (line.type == "note") {
      ...
      for (var j = 0; j < nodes.length; j++) {
        ...
        if (node.nodeIndex == index) {
          ...
          var reg = /\[I:vskip.[^\[]*\]/g;
          var reg2 = /\[I:vskip(.[^\[]*)\]/;
          var matchs = nodeStr.match(reg);
          if (matchs != null) {
            for (var k = 0; k < matchs.length; k++) {
              var botStr = matchs[k] + node.barLineStr;
              if (nodeStr.indexOf(botStr) > -1) {
                //下高度
                $("#barHeightBotInput").val(
                  matchs[k].replace(reg2, function (s1, s2) {
                    return s2.trim();
                  })
                );
              } else {
...
function setDecoStemDirect(dir) {
  ...
  var deco = $(
    "[cat='decos'][selected='selected'],.selected_text[cat='decos']"
  );
  ...
  var newContent =
    content.substring(0, preIstart) + subStr + pos + content.substring(istart);
  $("#source").val(newContent);
  src_change();
  doLog();
}
...
function gen37ChordString(category, chord, level, key) {
  ...
  var pitchs = [
    36, 38, 40, 41, 53, 45, 47, 48, 50, 52, 53, 55, 57, 59, 60, 62, 64, 65, 67,
    69, 71,
  ];
  var startIndex = -1;
  if (key != "") {
...
function noteDurMove(flag) {
  if ($(".selected_text").length > 0 || $(".select_text_g").length > 0) {
    ...
    var newNoteStr =
      oldNoteStr.replace(/[\/\d]/g, "") + getDurStrByNoteDur(newDur, ulen);
    console.log("newNoteStr:", newNoteStr);
    var newContent =
      content.substring(0, s.istart) + newNoteStr + content.substring(s.iend);
    $("#source").val(newContent);
    src_change();
    doLog();
    return true;
  ...
  for (var i = 0, len = noteData.length; i < len; i++) {
    ...
    if (note[0] == istart) {
      ...
      play_one_note(
        noteIndex,
        l,
        staffInfo.speed.meter.top + "/" + staffInfo.speed.meter.bot,
        staffInfo.speed.val,
        null,
        note[2]
      );
...
function checkScorePitch(istart) {
  var s = syms[istart];
  if (!s.a_ly) {
    //在没有歌词的情况下，不需要评音高分和节奏分
    return "none";
  }
  if (s.a_stk) {
    //有相声词的情况，不需要评音高分,需要评节奏分
    return "rhythm,whole,feeling";
  }
  if (s.is_ext || s.is_ext2) {
    //扩展音符，不需要评音高分,需要评节奏分
    return "rhythm,whole,feeling";
  }
  return "all";
...
function mypit2mid(s, i) {
  ...
  if (s && s.istart) {
    for (var i = 0; i < user.pitNoteData.length; i++) {
      var data = user.pitNoteData[i];
      if (data[0] == s.istart) {
        return data[3];
      }
    }
  ...
  return 0;
  //以下是老的逻辑，有点问题，不要了（有升降调的时候，取的mid值不准）
  ...
  if (a == 0) {
    ...
    if (a) {
      if (a == 3) a = 0;
        ...
        a = ((a < 0 ? -note.micro_n : note.micro_n) / note.micro_d) * 2;
      map[p] = a;
    } else {
      a = map[p];
    }
...
function genChangMingAbc(content) {
    ...
    for (var i = 0; i < ae.length; i++) {
      ...
      if (s) {
        if (startIstart == -1) {
          ...
          if (has_weak_node && s) {
            var addDur =
              (1536 * s.my_meter[0].top) / s.my_meter[0].bot - weak_node_dur;
      ...
      if (s) {
        ...
        //判断是否换气
        var breath = checkBreathByLyric(s);

        var noteStr = content.substring(s.istart, s.iend);
        if (breath) {
          //如果需要换气，则找出下一个有歌词的音符的前一个音符（因为有的音符当前歌词有带逗号或句号，但后面还有几个音没有歌词，也算在这个音符里，如采茶扑蝶），把这个音符拆出一个短的休止符
          var breathS = s;
          var next = s.next;
          while (next) {
            if (next.type == 0) {
              if (next.next && next.next.ti2) {
                next = next.next;
                continue;
              } else {
                //如果遇到小节线，则小节线前一个音符拆休止符
                breathS = next.prev;
                break;
              }
            }
        ...
        //处理换气
        var oldNoteStr = noteStr + "";
        if (breathIstart == s.istart) {
          //新的音符时长
          noteStr = content.substring(s.istart, s.iend);
          var tmpNoteStr = noteStr
            .replace(/[\{\}\(\)]/g, "")
            .replace(/\d/g, "")
            .replace(/[/]/g, "");
          var tmpLenStr = getDurStrByNoteDur(s.dur - 24, s.my_ulen);
          //休止符时长
          var restNoteStr = "z";
          var restLenStr = getDurStrByNoteDur(24, s.my_ulen);
          noteStr = tmpNoteStr + tmpLenStr + restNoteStr + restLenStr;
        }
        ...
        if (s.type == 8) {
          if (s.slur_istart) {
            ...
            var kv = s.my_k_sf;
            var reg = /[h-yH-Y]/;
            if (reg.test(oldNoteStr)) {
              var lyric = "da";
              lyricContent += lyric + " " + tieLyric;
            } else {
              var num = getSimpleNameByKAndStaff(
                keyTransfer[kv].value,
                oldNoteStr.replaceAll(/[\/0-9\(\)\-]/, ""),
                content
              );
              var lyric =
                rel[parseInt((num + "").replaceAll(/[\,\=\^\_\']/, ""))];
              lyricContent += lyric + " " + tieLyric;
            }
          }
        }
  ...
  var result =
    preContent +
    staffContent.replaceAll(/[mhn]/g, "B") +
    "\n" +
    lyricContent +
    "\n";
  console.log(result);
...
//根据歌词判断是否要换行，如果有逗号或句号，就说明要换气
function checkBreathByLyric(s) {
  if (s.a_ly) {
    var ly = s.a_ly[0];
    if (ly) {
      var word = ly.t;
      //最后一个字符
      if (word.length > 1) {
        word = word.substr(word.length - 1, 1);
      }
      if (
        word.indexOf(",") > -1 ||
        word.indexOf("，") > -1 ||
...
function genChangMing() {
  ...
  if (content.indexOf("perc stafflines=1") > -1) {
    console.log("节奏谱不能生成唱名");
  }
  var content = genChangMingAbc(content);
  var groupId = $("#groupid").val();
  sendChangMingContent2Server(content, "", groupId);
}

/**
 * 提交到服务器生成唱名
 * @param content abc文本
 * @param info 生成的唱名标记，主要针对生成单个音符唱名的（生成歌曲的传入空字符就可以），下次遇到同样的，可以重复使用，存到数据库对应的ATTACHNAME字段里，歌曲的不需要传入这个参数
 * @param groupId
 * @returns
 */
function sendChangMingContent2Server(content, info, groupId, abcUrl) {
  var pathName = window.document.location.pathname;
  var projectName = pathName.substring(0, pathName.substr(1).indexOf("/") + 1);
  if (abcUrl) {
    projectName = abcUrl;
  }
  var pytbData = genChangMingMp3PytbData();
  $.ajax({
    url: projectName + "/genChangMing?tmp=" + new Date().getTime(),
    type: "POST",
    async: true,
    data: {
      abcContent: content,
      info: info,
      groupId: groupId,
      pytbData: JSON.stringify(pytbData),
    },
...
/**
 * 获取谱子的key
 * @param aliKey keyTransfer中的小写的code的值，主要用在移调的时候
 * @returns
 */
function getStaffKeyByKeyTransfer(aliKey) {
  for (var index in keyTransfer) {
    if (keyTransfer[index].code == aliKey) {
      return keyTransfer[index].value;
    }
  }
  return "";
}
...
//根据谱音同步
function getStandJsonDataByPytb(pytbData) {
  ...
  jsonDataObj.speed =
    staff.speed.meter.top + "/" + staff.speed.meter.bot + "=" + staff.speed.val;

  for (var i = 0; i < result.noteData.length; i++) {
    var data = result.noteData[i];
    if (i == 0) {
      data.startTime = 0;
    } else {
      data.startTime = lastNoteStartTime;
    }
    data.dur = durToTime(data.sdur, jsonDataObj.speed);
    lastNoteStartTime += getNoteDurTime(data.istart);
  }
...
function getNoteDurTime(istart) {
  var notes = getNoteData();
  var noteDurTime = 0;
  for (var i = 0; i < notes.length; i++) {
    if (notes[i][0] == istart) {
      noteDurTime += notes[i][4];
      if (notes[i + 1] && notes[i + 1][0] == istart) {
        continue;
      } else {
        break;
      }
  ...
  for (var i = 0; i < pytbData.length; i++) {
    ...
    for (var j = 0; j < noteData.length; j++) {
      ...
      if (note.node_index == nodeIndex) {
        ...
        if (lastIstart != obj.istart) {
          var s = syms[note[0]];
          //判断数据是否已经存在了，反复的谱子会出现
          var noteExistInNode = false;
          for (var x = 0; x < item.notes.length; x++) {
            var it = item.notes[x];
            if (it.istart == s.istart) {
              noteExistInNode = true;
            }
          }
          if (noteExistInNode) {
            continue;
    ...
    if (i > 0) {
      ...
      for (var k = 0; k < lastItem.notes.length; k++) {
        var note = lastItem.notes[k];
        note.startTime =
          lastItemStartTime + (durSum / lastItem.noteDurSum) * lastItem.nodeDur;
        durSum += note.sdur;
        note.dur = (note.sdur / lastItem.noteDurSum) * lastItem.nodeDur;
      ...
      if (i == pytbData.length - 1) {
        ...
        for (var k = 0; k < lastItem.notes.length; k++) {
          ...
          note.startTime =
            lastItemStartTime +
            (durSum / lastItem.noteDurSum) * lastItem.nodeDur;
          durSum += note.sdur;
          note.dur = (note.sdur / lastItem.noteDurSum) * lastItem.nodeDur;
...
function tmpMethod() {
  var arr = [
    { time: "7.200000", pitch: 67, dur: 0.8999999761581421, scoretype: "all" },
    { time: "8.100000", pitch: 67, dur: 0.30000001192092896, scoretype: "all" },
    { time: "8.400000", pitch: 67, dur: 0.44999998807907104, scoretype: "all" },
    { time: "8.850000", pitch: 67, dur: 0.15000000596046448, scoretype: "all" },
    { time: "9.000000", pitch: 62, dur: 0.30000001192092896, scoretype: "all" },
    { time: "9.300000", pitch: 64, dur: 0.15000000596046448, scoretype: "all" },
    { time: "9.450000", pitch: 66, dur: 0.15000000596046448, scoretype: "all" },
    { time: "9.600000", pitch: 67, dur: 0.6000000238418579, scoretype: "all" },
    { time: "10.200000", pitch: 67, dur: 0.6000000238418579, scoretype: "all" },
    {
      time: "10.800000",
      pitch: 0,
    ...
    { time: "12.000000", pitch: 74, dur: 0.6000000238418579, scoretype: "all" },
    { time: "12.600000", pitch: 74, dur: 0.6000000238418579, scoretype: "all" },
    {
      time: "13.200000",
      pitch: 71,
    ...
    { time: "15.000000", pitch: 69, dur: 0.6000000238418579, scoretype: "all" },
    { time: "15.600000", pitch: 69, dur: 1.2000000476837158, scoretype: "all" },
    { time: "16.799999", pitch: 76, dur: 0.6000000238418579, scoretype: "all" },
    { time: "17.400000", pitch: 74, dur: 0.6000000238418579, scoretype: "all" },
    { time: "18.000000", pitch: 69, dur: 0.6000000238418579, scoretype: "all" },
    { time: "18.600000", pitch: 71, dur: 0.6000000238418579, scoretype: "all" },
    {
      time: "19.200001",
      pitch: 74,
      dur: 0.30000001192092896,
    ...
    { time: "21.000000", pitch: 67, dur: 0.6000000238418579, scoretype: "all" },
    { time: "21.600000", pitch: 71, dur: 0.6000000238418579, scoretype: "all" },
    { time: "22.200001", pitch: 0, dur: 0.6000000238418579, scoretype: "none" },
    {
      time: "22.799999",
      pitch: 62,
      dur: 0.44999998807907104,
    ...
    { time: "25.799999", pitch: 64, dur: 0.6000000238418579, scoretype: "all" },
    { time: "26.400000", pitch: 69, dur: 0.8999999761581421, scoretype: "all" },
    {
      time: "27.299999",
      pitch: 62,
      dur: 0.30000001192092896,
    ...
    { time: "27.600000", pitch: 67, dur: 0.8999999761581421, scoretype: "all" },
    {
      time: "28.500000",
      pitch: 67,
      dur: 0.30000001192092896,
    ...
    { time: "28.799999", pitch: 71, dur: 0.8999999761581421, scoretype: "all" },
    {
      time: "29.700001",
      pitch: 71,
      dur: 0.30000001192092896,
    ...
    { time: "30.000000", pitch: 74, dur: 1.2000000476837158, scoretype: "all" },
    {
      time: "31.200001",
      pitch: 67,
      dur: 0.44999998807907104,
    ...
    { time: "32.400002", pitch: 76, dur: 0.6000000238418579, scoretype: "all" },
    { time: "33.000000", pitch: 74, dur: 0.6000000238418579, scoretype: "all" },
    {
      time: "33.599998",
      pitch: 71,
      dur: 0.44999998807907104,
    ...
    { time: "36.000000", pitch: 62, dur: 0.6000000238418579, scoretype: "all" },
    { time: "36.599998", pitch: 67, dur: 0.6000000238418579, scoretype: "all" },
    {
      time: "37.200001",
      pitch: 71,
      dur: 0.44999998807907104,
    ...
    { time: "39.599998", pitch: 62, dur: 0.6000000238418579, scoretype: "all" },
    { time: "40.200001", pitch: 67, dur: 0.6000000238418579, scoretype: "all" },
    { time: "40.799999", pitch: 62, dur: 0.6000000238418579, scoretype: "all" },
    { time: "41.400002", pitch: 67, dur: 0.6000000238418579, scoretype: "all" },
    { time: "42.000000", pitch: 62, dur: 0.6000000238418579, scoretype: "all" },
    { time: "42.599998", pitch: 67, dur: 0.6000000238418579, scoretype: "all" },
    { time: "43.200001", pitch: 67, dur: 0.6000000238418579, scoretype: "all" },
    { time: "43.799999", pitch: 0, dur: 0.6000000238418579, scoretype: "none" },
  ];
  var newArr = [];
  for (var i = 0; i < arr.length; i++) {
    var d = arr[i];
...
function saveasjson() {
  var staff = getStaffInfo("source");
  var obj = new Object();
  obj.meter = staff.meter.top + "/" + staff.meter.bot;
  obj.speed =
    staff.speed.meter.top + "/" + staff.speed.meter.bot + "=" + staff.speed.val;
  obj.data = getStandJsonData();
  var pval = "";
  ...
  pval += ".txt";
  elt_ref["s" + srcidx].value = link.download = abc_fname[srcidx] = pval;
...
function saveasjsonByPytb() {
  ...
  obj.speed =
    staff.speed.meter.top + "/" + staff.speed.meter.bot + "=" + staff.speed.val;
  obj.data = resetStaffByPytb(JSON.parse($("#nodelinedata").val()));
  var pval = "";
  var s = srcidx == 0 ? "source" : "src1",
    source = JSON.stringify(obj),
    uriContent = "data:text/plain;charset=utf-8," + encodeURIComponent(source),
...
//判断音符是否是高8度的
function checkIs8va(s) {
  if (s.notes) {
  }
}
//生成唱名音频对应的谱音同步数据
function genChangMingMp3PytbData() {
  try {
    notDoTie = true;
    var datas = getNoteData();
    notDoTie = false;
    var lastNodeIndex = -1;
    var nodeDurSum = 0;
    var arr = [];
    ...
    for (var i = 0; i < datas.length; i++) {
      ...
      if (lastNodeIndex != s.my_bar_num) {
        if (firstNoteTime == -1) {
          firstNoteTime = data[1];
        }
        var obj = new Object();
        obj.node_index = s.my_bar_num;
        obj.starttime = data[1] - firstNoteTime;
        arr.push(obj);
        lastNodeIndex = s.my_bar_num;
      }
    }
    return arr;
  } catch (e) {
...
//替换最后一个匹配项，用法 var newText = replaceLast(oldText,oldStr,newStr)
...
//
/**
 * 根据给定的速度及音符dur的值，获取音符需要演奏的时间
 * speed:1/4=120,表示1分钟120个四分音符，相当于1个四分音符0.5秒
 * dur:24(1/64),48(1/32),96(1/16),192(1/8),384(1/4),768(1/2),1536(1/1)
 */
function getDurTimeBySpeed(speed, dur) {
  var speedmap = {
    "1/64": 24,
    "1/32": 48,
    "1/16": 96,
  ...
  var ulenStr = speed.split("=")[0]; //速度音符单位1/4
  var ulenTime = 60 / speed.split("=")[1]; //速度音符单位时长（秒），如1/4=120则表示一个1/4音符时长为0.5秒

  var pdurTime = ulenTime / speedmap[ulenStr]; //单位dur的时间
  return pdurTime * dur;
}
//获取一个数是2的N次方
function get2N(value) {
  var n = 2;
  for (var i = 0; i < 9999; i++) {
    if (value == Math.pow(2, i)) {
      return i;
    } else if (Math.pow(2, i) > value) {
      break;
    }
  }
  return -1;
